          ...
          <div className="flex-1 bg-slate-950 p-4 sm:p-8 overflow-y-auto custom-scrollbar flex justify-center items-start">
            ...
            <div
              ...
              className={`bg-white text-black font-sans shadow-2xl border border-gray-400 transition-all ${
                orientation === 'landscape'
                  ? 'w-full max-w-[1100px] min-h-[690px] p-3.5 sm:p-4 text-[8px]'
                  : 'w-full max-w-[800px] min-h-[1050px] p-6 text-[9px]'
              }`}
              style={{
                color: '#000000',
            ...
            >
              ...
              <div className={`border-b-2 border-black ${orientation === 'landscape' ? 'pb-1 mb-2' : 'pb-2 mb-3'}`}>
                <div className={`flex items-start justify-between border-b border-black ${orientation === 'landscape' ? 'pb-1 mb-1' : 'pb-1.5 mb-1.5'}`}>
                  <div className="text-left">
                    <div className={`${orientation === 'landscape' ? 'text-[9.5px]' : 'text-[11px]'} font-black uppercase tracking-wider text-black`}>
                      {institutionName}
                    </div>
                    <div className={`${orientation === 'landscape' ? 'text-[7.5px]' : 'text-[9px]'} font-bold text-gray-700`}>
                      YKS AKADEMİK BAŞARI VE DERS ÇALIŞMA TAKİP SİSTEMİ
                    </div>
                  </div>

                  <div className="text-right">
                    <div className={`${orientation === 'landscape' ? 'text-[8.5px]' : 'text-[10px]'} font-black text-black`}>
                      HAFTA: <span className="font-mono">{weekLabel}</span>
                    </div>
                    <div className={`${orientation === 'landscape' ? 'text-[7.5px]' : 'text-[9px]'} text-gray-600`}>
                      Çıktı Tarihi: {new Date().toLocaleDateString('tr-TR')}
                    </div>
                  </div>
                </div>

                <div className={`text-center ${orientation === 'landscape' ? 'my-0.5' : 'my-1'}`}>
                  <h1 className={`${orientation === 'landscape' ? 'text-xs sm:text-sm' : 'text-base sm:text-lg'} font-black tracking-tight text-black uppercase`}>
                    {documentTitle}
                  </h1>
                </div>

                {/* Öğrenci Bilgileri Şeridi */}
                <div className={`grid grid-cols-4 gap-1.5 bg-gray-100 border border-black ${orientation === 'landscape' ? 'p-1 text-[8px]' : 'p-2 text-[10px]'} font-bold`}>
                  <div>
                    <span className={`text-gray-600 block ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[8px]'} uppercase`}>Öğrenci Adı Soyadı</span>
                    <span className={`text-black font-black ${orientation === 'landscape' ? 'text-[9px]' : 'text-[11px]'}`}>{studentName}</span>
                  </div>
                  <div>
                    <span className={`text-gray-600 block ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[8px]'} uppercase`}>Alan / Hedef Sıralama</span>
                    <span className="text-black">{targetField} Alanı • {targetRank}</span>
                  </div>
                  <div>
                    <span className={`text-gray-600 block ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[8px]'} uppercase`}>Hedef Üniversite & Bölüm</span>
                    <span className="text-black truncate block">{targetUni.split('(')[0]} - {targetDept}</span>
                  </div>
                  <div>
                    <span className={`text-gray-600 block ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[8px]'} uppercase`}>Haftalık Toplam Hedef</span>
              ...
              <div className={orientation === 'landscape' ? 'mb-1.5' : 'mb-3'}>
                <div className={`grid ${orientation === 'landscape' ? 'grid-cols-7 gap-1' : 'grid-cols-1 sm:grid-cols-2 gap-2'}`}>
                  {DAYS.map((day) => {
                    ...
                    return (
                      <div
                        ...
                        className={`${
                          orientation === 'landscape'
                            ? 'border border-black min-h-[175px] text-[7.5px]'
                            : 'border-2 border-black min-h-[220px] text-[9px]'
                        } flex flex-col justify-between bg-white`}
                      >
                        {/* Gün Başlığı (Mürekkep Tasarruflu Açık Gri Zemin) */}
                        <div className={`bg-gray-200 text-black border-b border-black ${orientation === 'landscape' ? 'px-1 py-0.5' : 'px-1.5 py-1'} text-center font-black flex items-center justify-between`}>
                          <span className={`uppercase ${orientation === 'landscape' ? 'text-[8px]' : 'text-[9.5px]'} font-black`}>{day}</span>
                          {dayDateInfo && (
                            <span className={`${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[7.5px]'} text-gray-700 font-mono font-bold`}>
                        ...
                        {/* Günün Görevleri */}
                        <div className={`${orientation === 'landscape' ? 'p-0.5 space-y-0.5' : 'p-1 space-y-1'} flex-1 flex flex-col justify-start`}>
                          {dayPlans.length === 0 ? (
                            <div className={`text-center text-gray-400 italic ${orientation === 'landscape' ? 'py-2 text-[7px]' : 'py-4 text-[8.5px]'}`}>
                              Planlanan ders yok
                            </div>
                          ) : (
                            dayPlans.map((plan, pIdx) => (
                              <div
                                key={plan.id || pIdx}
                                className={`border-b border-gray-300 ${orientation === 'landscape' ? 'pb-0.5' : 'pb-1'} last:border-b-0`}
                              >
                                <div className="flex items-start space-x-1">
                                  {showCheckboxes && (
                                    <span className={`inline-block ${orientation === 'landscape' ? 'w-2.5 h-2.5' : 'w-3 h-3'} border border-black rounded-[2px] mt-0.5 shrink-0`} />
                                  )}
                                  <div className="min-w-0 flex-1">
                                    <div className={`font-black text-black ${orientation === 'landscape' ? 'text-[7.5px]' : 'text-[9px]'} leading-tight break-words`}>
                                      {plan.subject}
                                    </div>
                                    <div className={`text-gray-800 ${orientation === 'landscape' ? 'text-[7px] line-clamp-2' : 'text-[8px] truncate'} leading-tight break-words`}>
                                      {plan.topic}
                                    </div>
                                    <div className={`text-gray-600 ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[7.5px]'} font-mono mt-0.2`}>
                          ...
                          {showBlankLines && (
                            <div className={`${orientation === 'landscape' ? 'pt-0.5 space-y-1' : 'pt-1 space-y-1.5'} mt-auto border-t border-dashed border-gray-300 opacity-60`}>
                              <div className="flex items-center space-x-1">
                                {showCheckboxes && <span className={`${orientation === 'landscape' ? 'w-2 h-2' : 'w-2.5 h-2.5'} border border-gray-400 rounded-[2px] shrink-0`} />}
                                <div className={`${orientation === 'landscape' ? 'h-1.5' : 'h-2'} border-b border-gray-400 w-full`} />
                              </div>
                              <div className="flex items-center space-x-1">
                                {showCheckboxes && <span className={`${orientation === 'landscape' ? 'w-2 h-2' : 'w-2.5 h-2.5'} border border-gray-400 rounded-[2px] shrink-0`} />}
                                <div className={`${orientation === 'landscape' ? 'h-1.5' : 'h-2'} border-b border-gray-400 w-full`} />
                        ...
                        <div className={`bg-gray-50 border-t border-black ${orientation === 'landscape' ? 'px-1 py-0.5 text-[6.5px]' : 'px-1.5 py-1 text-[7.5px]'} space-y-0.5`}>
                          ...
                          <div className="flex items-center justify-between border-t border-dashed border-gray-300 pt-0.5 font-bold text-black">
                            <span className={`${orientation === 'landscape' ? 'text-[6px]' : 'text-[7px]'} uppercase tracking-tighter`}>Net Süre:</span>
                            <span className={`font-mono ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[7.5px]'} bg-white px-0.5 border border-gray-400 rounded`}>
                              ___ sa ___ dk
                            </span>
                          </div>
                        </div>
              ...
              {showRoutines && (
                <div className={`border border-black ${orientation === 'landscape' ? 'mb-1 text-[7.5px]' : 'mb-2 text-[8.5px]'}`}>
                  <div className={`bg-gray-200 ${orientation === 'landscape' ? 'px-1.5 py-0.5 text-[7px]' : 'px-2 py-0.5 text-[8px]'} font-black uppercase border-b border-black flex items-center justify-between`}>
                    <span>GÜNLÜK ALIŞKANLIK & DİSİPLİN TAKİP ÇİZELGESİ</span>
                    <span className={`${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[7.5px]'} font-normal text-gray-700`}>Her gün tamamlanan rutine [X] atınız</span>
                  </div>
                  
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className={`border-b border-black bg-gray-50 ${orientation === 'landscape' ? 'text-[7px]' : 'text-[8px]'} font-bold text-center`}>
                          <th className={`${orientation === 'landscape' ? 'p-0.5 w-32' : 'p-1 w-36'} border-r border-black text-left`}>Rutin Adı / Hedef</th>
                          {DAYS.map(d => (
                            <th key={d} className={`${orientation === 'landscape' ? 'p-0.5 w-8' : 'p-1 w-10'} border-r border-black last:border-r-0`}>
                              {d.slice(0, 3)}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {(routines.length > 0 ? routines : [
                          ...
                          <tr key={rot.id || rIdx} className="border-b border-gray-300 last:border-b-0 text-center">
                            <td className={`${orientation === 'landscape' ? 'p-0.5 text-[7px]' : 'p-1 text-[8px]'} border-r border-black text-left font-bold`}>
                              {rot.title} <span className="font-normal text-gray-600">({rot.target})</span>
                            </td>
                            {DAYS.map(d => (
                              <td key={d} className={`${orientation === 'landscape' ? 'p-0.5' : 'p-1'} border-r border-black last:border-r-0`}>
                                <span className={`inline-block ${orientation === 'landscape' ? 'w-2.5 h-2.5' : 'w-3.5 h-3.5'} border border-black rounded-[2px]`} />
                              </td>
                            ))}
                          </tr>
                        ))}
              ...
              <div className={`grid grid-cols-1 sm:grid-cols-2 ${orientation === 'landscape' ? 'gap-1.5 text-[7.5px]' : 'gap-2 text-[8.5px]'} mt-auto`}>
                {/* Koçluk Notu & Haftalık Hedef */}
                {showGoalSummary && (
                  <div className={`border border-black ${orientation === 'landscape' ? 'p-1.5' : 'p-2'} bg-gray-50 flex flex-col justify-between`}>
                    <div>
                      <div className={`font-black ${orientation === 'landscape' ? 'text-[7.5px]' : 'text-[8.5px]'} uppercase border-b border-gray-300 pb-0.5 mb-0.5 text-black`}>
                        Haftalık Odak & Koçluk Değerlendirmesi
                      </div>
                      <p className={`${orientation === 'landscape' ? 'text-[7px] line-clamp-2' : 'text-[8px]'} text-gray-800 leading-snug italic`}>
                        "{coachNote}"
                      </p>
                    </div>
                    <div className={`mt-0.5 pt-0.5 border-t border-dashed border-gray-300 ${orientation === 'landscape' ? 'text-[6.5px]' : 'text-[7.5px]'} text-gray-600 flex justify-between`}>
                ...
                {/* İmza ve Onay Bölümü */}
                {showSignatures && (
                  <div className={`border border-black ${orientation === 'landscape' ? 'p-1.5' : 'p-2'} bg-white flex flex-col justify-between`}>
                    <div className={`font-black ${orientation === 'landscape' ? 'text-[7.5px]' : 'text-[8.5px]'} uppercase border-b border-gray-300 pb-0.5 mb-0.5 text-black`}>
                      Onay & İmzalar
                    </div>
                    <div className={`grid grid-cols-3 gap-1 ${orientation === 'landscape' ? 'pt-1 text-[6.5px]' : 'pt-4 text-[7.5px]'} text-center`}>
                      <div>
                        <div className={`border-b border-black mb-0.5 ${orientation === 'landscape' ? 'h-2.5' : 'h-4'}`} />
              ...
              {/* Alt Küçük Dipnot */}
              <div className={`${orientation === 'landscape' ? 'text-[6.5px] mt-1 pt-0.5' : 'text-[7px] mt-2 pt-1'} text-gray-500 text-center border-t border-gray-200`}>
